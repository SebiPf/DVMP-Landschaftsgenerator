...
class OBJECT_OT_add_trees(bpy.types.Operator):
    ...
    GRID_LENGTH: bpy.props.IntProperty(
        name="Grid Size",
        description="Determine the size of the Grid-Area",
        default=50
    )
    ...
    def create_trees(self):
        for i in range(self.NUMBER_TREES):
            # Variables
            length = (
                self.getRandom(0.5, 1.2),
                self.getRandom(0.5, 0.9),
                0.5,
                0.1
            )
            downAngle = (
            ...
            # Create & append Materials
            mat_wood = self.create_wood_material()
            mat_leaf = self.create_leaf_material()
            ...
            # Add Materials
            tree.data.materials.append(mat_wood)
            leaf.data.materials.append(mat_leaf)

            # Set Location
            tree.location = (
                self.getRandom(0, self.GRID_LENGTH),
                self.getRandom(0, self.GRID_LENGTH),
                0.0
            )
